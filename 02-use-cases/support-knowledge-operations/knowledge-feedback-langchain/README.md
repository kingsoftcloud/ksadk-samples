# 客服知识运营 Agent - LangChain

客服知识运营 Agent 演示如何围绕知识缺口、工单反馈、内容更新、发布复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实客户、手机号、工单原文、账号信息、坐席工号、内部故障细节或未公开产品路线图。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现客服知识运营 Agent。
...
```bash
cd 02-use-cases/support-knowledge-operations/knowledge-feedback-langchain
cp ../../../.env.example .env
