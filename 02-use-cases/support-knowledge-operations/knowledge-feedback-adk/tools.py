...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为客服工单系统、知识库、坐席反馈、搜索日志、内容审核流和发布看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为客服工单系统、知识库、坐席反馈、搜索日志、内容审核流和发布看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责知识缺口、工单反馈、内容更新和发布复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [
        {"step": index, "owner": '客服知识运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"}
