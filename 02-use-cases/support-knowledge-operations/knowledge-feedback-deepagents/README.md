# 客服知识运营 Agent - DeepAgents

客服知识运营 Agent 演示如何围绕知识缺口、工单反馈、内容更新、发布复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实客户、手机号、工单原文、账号信息、坐席工号、内部故障细节或未公开产品路线图。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现客服知识运营 Agent。
...
- `知识缺口`：用公开虚构数据展示当前状态。
- `工单反馈`：把客服工单、知识命中和用户追问拆成可核验条目。
- `内容更新`：生成下一步行动，明确哪些需要人工复核。
- `发布复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/support-knowledge-operations/knowledge-feedback-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `知识缺口`、`工单反馈`、`内容更新` 和 `发布复盘`。未接入真实系统时，输出会明确标注为 fixture，不会伪造平台结果。
...
- 把 `OPERATIONS_DATA` 接入真实客服工单系统、知识库、坐席反馈、搜索日志、内容审核流和发布看板数据，但不要在样例输出中展示敏感字段。
