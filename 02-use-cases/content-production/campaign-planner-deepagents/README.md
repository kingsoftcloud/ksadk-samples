# 内容生产传播 Agent - DeepAgents

演示内容生产 Agent 如何围绕开源发布目标生成传播计划、渠道素材、内容草稿和审核清单。默认使用公开 fixture，不调用真实营销系统。 这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 Content Production Campaign Planner。
...
```bash
cd 02-use-cases/content-production/campaign-planner-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `创意简报`、`渠道计划`、`内容草稿` 和 `审核清单`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `CAMPAIGN_BRIEF` 替换为发布计划或产品 brief。
- 把 `CHANNELS` 接入 CMS、GitHub Release、博客平台或社媒排期系统。
- 把审核清单接入品牌规范、安全合规和事实核查流程。
