...
农业生产 Agent 如何围绕种植计划、环境数据、农事任务和产量预测生成公开可复现的农事生产计划。默认使用虚构农业 fixture，不包含真实地块、真实农户、真实产量或商业采购数据。 这是一个多文件 ADK Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 ADK 如何实现 农业生产农事计划 Agent。
...
```bash
cd 02-use-cases/agriculture-production/crop-planner-adk
cp ../../../.env.example .env
