# 能源调度负荷平衡 Agent - DeepAgents

能源调度 Agent 如何根据负荷预测、设备状态和安全边界生成调度策略。默认使用公开虚构数据，不连接真实电网、场站或控制系统。 这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 Energy Dispatch Load Balancer Agent。
...
- `负荷预测`：展示未来时段负荷、峰谷变化和天气影响。
- `设备状态`：整理储能、光伏、变压器和备用容量的可用状态。
- `调度策略`：生成削峰填谷、储能充放和需求响应建议。
- `安全边界`：强调人工确认、保护定值和控制系统隔离。
...
```bash
cd 02-use-cases/energy-dispatch/load-balancer-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `负荷预测`、`设备状态`、`调度策略` 和 `安全边界`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `LOAD_FORECAST` 替换为 EMS、计量平台、天气预测和负荷模型。
- 把 `EQUIPMENT_STATUS` 接入设备台账、储能 BMS、逆变器和告警系统。
- 把调度建议写回调度工单或人工确认看板；不要直连真实控制系统。
