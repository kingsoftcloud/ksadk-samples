...
def collect_report_sources(query: str) -> list[dict]:
    ...
    ADK 运行时会把这个函数作为工具暴露给 root_agent；demo.py 也会直接调用它做离线演示。
    """
...
{goal_line} 当前 ADK 版本使用函数工具组织报告材料，适合对比 LangGraph 版本的显式工作流写法。
...
ADK 版本强调把报告材料检索做成可调用工具，Agent 可以在真实模型运行时主动调用 `collect_report_sources`，再基于引用材料输出报告。默认离线 demo 不访问外部搜索，避免伪造结果。

## 工程说明

- `agent.py` 暴露 ADK `root_agent`。
- `tools.py` 同时服务 ADK 工具调用和离线 demo。
