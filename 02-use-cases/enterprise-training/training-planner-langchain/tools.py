...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为LMS、HR 系统、课程库、考试平台、练习环境和学习数据看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为LMS、HR 系统、课程库、考试平台、练习环境和学习数据看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责培训对象、课程设计、练习评估和学习复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [
        {"step": index, "owner": '培训运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"}
