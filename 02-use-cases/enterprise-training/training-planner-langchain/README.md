# 企业培训计划 Agent - LangChain

企业培训计划 Agent 演示如何围绕培训对象、课程设计、练习评估、学习复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实员工、工号、绩效、考试成绩、个人评价、薪酬信息或企业内部课程资料。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现企业培训计划 Agent。
...
```bash
cd 02-use-cases/enterprise-training/training-planner-langchain
cp ../../../.env.example .env
