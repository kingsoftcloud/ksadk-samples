# 企业培训计划 Agent - DeepAgents

企业培训计划 Agent 演示如何围绕培训对象、课程设计、练习评估、学习复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实员工、工号、绩效、考试成绩、个人评价、薪酬信息或企业内部课程资料。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现企业培训计划 Agent。
...
- `培训对象`：用公开虚构数据展示当前状态。
- `课程设计`：把学员画像、课程和练习数据拆成可核验条目。
- `练习评估`：生成下一步行动，明确哪些需要人工复核。
- `学习复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/enterprise-training/training-planner-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `培训对象`、`课程设计`、`练习评估` 和 `学习复盘`。未接入真实系统时，输出会明确标注为 fixture，不会伪造平台结果。
...
- 把 `OPERATIONS_DATA` 接入真实LMS、HR 系统、课程库、考试平台、练习环境和学习数据看板数据，但不要在样例输出中展示敏感字段。
