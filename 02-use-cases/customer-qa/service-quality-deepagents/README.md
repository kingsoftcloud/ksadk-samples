# 客服质检改进 Agent - DeepAgents

客服质检 Agent 如何围绕质检样本、规则命中、改进建议和培训闭环生成公开可复现的服务质量改进方案。 默认使用公开虚构客服质检 fixture，不包含真实通话录音、聊天原文、客户身份、坐席工号或投诉详情。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现客服质检改进 Agent。
...
- `质检样本`：用公开虚构数据展示当前状态。
- `规则命中`：把资源、服务或环境数据拆成可核验条目。
- `改进建议`：生成下一步行动，明确哪些需要人工复核。
- `培训闭环`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/customer-qa/service-quality-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `质检样本`、`规则命中`、`改进建议` 和 `培训闭环`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为质检样本、规则版本、命中证据、坐席反馈和培训计划中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实质检聚合标签、规则命中和复检结果数据，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回质检复盘、培训计划、知识库更新任务或坐席辅导记录；关键结论必须保留人工确认。
