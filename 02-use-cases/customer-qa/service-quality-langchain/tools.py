...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为客服工单、质检规则库、知识库、培训系统、坐席反馈和质量看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为客服工单、质检规则库、知识库、培训系统、坐席反馈和质量看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责质检样本、规则命中、改进建议和培训闭环。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '客服质检负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
