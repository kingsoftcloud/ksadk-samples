# 客服质检改进 Agent - LangChain

客服质检 Agent 如何围绕质检样本、规则命中、改进建议和培训闭环生成公开可复现的服务质量改进方案。 默认使用公开虚构客服质检 fixture，不包含真实通话录音、聊天原文、客户身份、坐席工号或投诉详情。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现客服质检改进 Agent。
...
```bash
cd 02-use-cases/customer-qa/service-quality-langchain
cp ../../../.env.example .env
