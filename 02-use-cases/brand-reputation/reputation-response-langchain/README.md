# 品牌舆情回应 Agent - LangChain

品牌舆情回应 Agent 演示如何围绕舆情线索、传播路径、回应策略、品牌复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实用户、手机号、社媒账号、私信内容、未公开声明、客户投诉原文或内部公关策略。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现品牌舆情回应 Agent。
...
```bash
cd 02-use-cases/brand-reputation/reputation-response-langchain
cp ../../../.env.example .env
