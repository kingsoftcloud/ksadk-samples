# 品牌舆情回应 Agent - DeepAgents

品牌舆情回应 Agent 演示如何围绕舆情线索、传播路径、回应策略、品牌复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实用户、手机号、社媒账号、私信内容、未公开声明、客户投诉原文或内部公关策略。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现品牌舆情回应 Agent。
...
- `舆情线索`：用公开虚构数据展示当前状态。
- `传播路径`：把社媒、客服和媒体线索拆成可核验条目。
- `回应策略`：生成下一步行动，明确哪些需要人工复核。
- `品牌复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/brand-reputation/reputation-response-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `舆情线索`、`传播路径`、`回应策略` 和 `品牌复盘`。未接入真实系统时，输出会明确标注为 fixture，不会伪造平台结果。
...
- 把 `OPERATIONS_DATA` 接入真实舆情监测平台、客服工单、社媒排期、品牌素材库、法务审核和公关复盘看板数据，但不要在样例输出中展示敏感字段。
