# 供应链需求计划 Agent - LangGraph

供应链计划 Agent 如何把需求预测、库存风险、调拨计划和监控指标组织成补货与调拨建议。默认使用公开虚构数据，不访问真实供应链系统。 这是一个多文件 LangGraph Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangGraph 如何实现 Supply Chain Demand Planner Agent。
...
```bash
cd 02-use-cases/supply-chain-planning/demand-planner-langgraph
cp ../../../.env.example .env
