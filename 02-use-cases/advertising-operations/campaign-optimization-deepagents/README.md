# 广告投放优化 Agent - DeepAgents

广告投放 Agent 如何围绕投放目标、素材审核、预算节奏和效果复盘生成公开可复现的投放优化方案。 默认使用公开虚构广告投放 fixture，不包含真实账户 ID、真实转化明细、用户画像、广告后台截图或未公开素材。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现广告投放优化 Agent。
...
- `投放目标`：用公开虚构数据展示当前状态。
- `素材审核`：把资源、服务或环境数据拆成可核验条目。
- `预算节奏`：生成下一步行动，明确哪些需要人工复核。
- `效果复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/advertising-operations/campaign-optimization-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `投放目标`、`素材审核`、`预算节奏` 和 `效果复盘`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为投放目标、渠道、素材、预算和转化漏斗中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实投放聚合指标、素材审核状态和落地页表现数据，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回投放调整单、素材审核任务、预算节奏表或效果复盘文档；关键结论必须保留人工确认。
