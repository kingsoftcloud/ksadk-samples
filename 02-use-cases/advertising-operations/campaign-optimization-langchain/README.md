# 广告投放优化 Agent - LangChain

广告投放 Agent 如何围绕投放目标、素材审核、预算节奏和效果复盘生成公开可复现的投放优化方案。 默认使用公开虚构广告投放 fixture，不包含真实账户 ID、真实转化明细、用户画像、广告后台截图或未公开素材。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现广告投放优化 Agent。
...
```bash
cd 02-use-cases/advertising-operations/campaign-optimization-langchain
cp ../../../.env.example .env
