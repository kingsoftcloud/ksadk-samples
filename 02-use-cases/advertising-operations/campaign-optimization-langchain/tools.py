...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为广告投放平台、素材库、落地页分析、转化归因、品牌审核和预算看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为广告投放平台、素材库、落地页分析、转化归因、品牌审核和预算看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责投放目标、素材审核、预算节奏和效果复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '广告投放负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
