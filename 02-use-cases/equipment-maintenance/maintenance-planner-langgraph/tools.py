...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为EAM、CMMS、点检系统、传感器平台、备件库存和生产排程。
    """
...
def _render_summary() -> str:
    return (
        f"- 设备概览：{SCENARIO_CONTEXT['target']}\n"
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为EAM、CMMS、点检系统、传感器平台、备件库存和生产排程。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责设备状态、备件计划、维修任务和停机风险。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": "设备维护计划员", "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
