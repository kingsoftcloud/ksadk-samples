# 设备维护维修计划 Agent - DeepAgents

设备维护 Agent 如何围绕设备状态、备件计划、维修任务和停机风险生成公开可复现的维修协同计划。 默认使用公开虚构设备维护 fixture，不包含真实产线、真实设备序列号、真实供应商或生产工艺数据。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 设备维护维修计划 Agent。
...
- `设备状态`：用公开虚构数据展示当前状态。
- `备件计划`：把资源、服务或环境数据拆成可核验条目。
- `维修任务`：生成下一步行动，明确哪些需要人工复核。
- `停机风险`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/equipment-maintenance/maintenance-planner-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `设备状态`、`备件计划`、`维修任务` 和 `停机风险`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为设备台账、传感器趋势、维修历史、备件库存和停机窗口中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实设备状态和备件可用性，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回维修工单、备件调拨单、停机审批或复盘报告；关键结论必须保留人工确认。
