# 设备维护维修计划 Agent - LangChain

设备维护 Agent 如何围绕设备状态、备件计划、维修任务和停机风险生成公开可复现的维修协同计划。 默认使用公开虚构设备维护 fixture，不包含真实产线、真实设备序列号、真实供应商或生产工艺数据。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现 设备维护维修计划 Agent。
...
```bash
cd 02-use-cases/equipment-maintenance/maintenance-planner-langchain
cp ../../../.env.example .env
