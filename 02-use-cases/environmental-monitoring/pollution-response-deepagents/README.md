# 环境监测污染响应 Agent - DeepAgents

环境监测 Agent 如何围绕监测点位、污染线索、处置联动和公众披露生成公开可复现的污染响应方案。 默认使用公开虚构环境监测 fixture，不包含真实企业排污数据、真实点位坐标、执法记录、企业名单或敏感环境事件。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现环境监测污染响应 Agent。
...
- `监测点位`：用公开虚构数据展示当前状态。
- `污染线索`：把资源、服务或环境数据拆成可核验条目。
- `处置联动`：生成下一步行动，明确哪些需要人工复核。
- `公众披露`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/environmental-monitoring/pollution-response-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `监测点位`、`污染线索`、`处置联动` 和 `公众披露`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为监测点位、污染指标、气象条件、处置资源和披露记录中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实监测摘要、复测结果和公众反馈数据，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回环保工单、巡查任务、公众披露草稿或污染事件复盘文档；关键结论必须保留人工确认。
