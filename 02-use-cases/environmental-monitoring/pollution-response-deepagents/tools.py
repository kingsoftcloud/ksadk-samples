...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为监测点位平台、IoT 传感器、环保工单、巡查记录、气象服务和公众披露平台。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为监测点位平台、IoT 传感器、环保工单、巡查记录、气象服务和公众披露平台。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责监测点位、污染线索、处置联动和公众披露。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '环境监测负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
