# 环境监测污染响应 Agent - LangChain

环境监测 Agent 如何围绕监测点位、污染线索、处置联动和公众披露生成公开可复现的污染响应方案。 默认使用公开虚构环境监测 fixture，不包含真实企业排污数据、真实点位坐标、执法记录、企业名单或敏感环境事件。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现环境监测污染响应 Agent。
...
```bash
cd 02-use-cases/environmental-monitoring/pollution-response-langchain
cp ../../../.env.example .env
