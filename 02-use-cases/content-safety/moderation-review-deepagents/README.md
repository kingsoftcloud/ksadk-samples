# 内容安全审核 Agent - DeepAgents

内容安全审核 Agent 如何围绕内容样本、规则命中、人工复核和风险处置生成公开可复现的审核方案。默认使用公开虚构 fixture，不包含真实用户、昵称、原文、截图、视频、举报人、敏感词库或风控阈值。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现内容安全审核 Agent。
...
```bash
cd 02-use-cases/content-safety/moderation-review-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `内容样本`、`规则命中`、`人工复核` 和 `风险处置`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为脱敏内容样本、规则命中摘要、复核队列、处置结果和申诉反馈中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实内容审核平台、举报系统、规则引擎、人工复核台、运营通知和样本回灌管道数据，但不要在样例输出中展示敏感字段。
