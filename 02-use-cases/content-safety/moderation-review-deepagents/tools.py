...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为内容审核平台、举报系统、规则引擎、人工复核台、运营通知和样本回灌管道。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为内容审核平台、举报系统、规则引擎、人工复核台、运营通知和样本回灌管道。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责内容样本、规则命中、人工复核和风险处置。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '内容安全负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
