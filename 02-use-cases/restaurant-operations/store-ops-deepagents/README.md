# 餐饮门店运营 Agent - DeepAgents

餐饮运营 Agent 如何围绕门店排班、食安巡检、库存损耗和顾客反馈生成公开可复现的门店协同方案。 默认使用公开虚构餐饮运营 fixture，不包含真实门店流水、会员手机号、员工信息、供应商价格或监管检查记录。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现餐饮门店运营 Agent。
...
```bash
cd 02-use-cases/restaurant-operations/store-ops-deepagents
cp ../../../.env.example .env
