...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为POS、排班系统、食安巡检、库存系统、外卖平台和顾客反馈看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为POS、排班系统、食安巡检、库存系统、外卖平台和顾客反馈看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责门店排班、食安巡检、库存损耗和顾客反馈。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '餐饮运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
