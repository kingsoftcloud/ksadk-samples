...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为CMS、热点监测、版权库、审核流、社媒排期和数据看板。
    """
...
def _render_summary() -> str:
    return (
        f"- 内容计划：{SCENARIO_CONTEXT['target']}\n"
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为CMS、热点监测、版权库、审核流、社媒排期和数据看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责内容排期、热点监测、版权风险和多渠道发布。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '媒体运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
