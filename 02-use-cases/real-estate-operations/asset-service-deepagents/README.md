# 房地产资产服务运营 Agent - DeepAgents

房地产运营 Agent 如何围绕资产状态、租户服务、工单协同和收益风险生成公开可复现的运营协同方案。默认使用虚构园区 fixture，不包含真实租户、真实楼宇、真实合同或真实财务数据。 这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 房地产资产服务运营 Agent。
...
```bash
cd 02-use-cases/real-estate-operations/asset-service-deepagents
cp ../../../.env.example .env
