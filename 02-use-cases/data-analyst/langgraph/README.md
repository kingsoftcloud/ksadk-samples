# 数据分析 Agent（Data Analyst）- LangGraph

把业务问题转成指标口径、样例数据分析、洞察和下一步实验。这是一个多文件 LangGraph Agent 工程，不是单文件脚本；默认使用本地公开示例数据，所以 clone 后不配置模型也能看到完整演示。

## 适用场景

- 想学习如何把 Data Analyst 写成可维护的 KSADK 工程。
...
```bash
cd 02-use-cases/data-analyst/langgraph
cp ../../../.env.example .env
...
- `分析这个月 Agent 调试功能的使用趋势。`
- `帮我找出样例仓库 Star 转化可能受哪些指标影响。`
