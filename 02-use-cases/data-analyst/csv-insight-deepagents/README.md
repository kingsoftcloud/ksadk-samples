# 数据分析 Agent（CSV 洞察）- DeepAgents

演示 Data Analyst Agent 如何把 CSV 样本转成指标口径、洞察排序和图表建议。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认使用本地公开 fixture，所以 clone 后不配置数据平台也能看到完整分析流程。
...
```bash
cd 02-use-cases/data-analyst/csv-insight-deepagents
cp ../../../.env.example .env
