...
这个 demo 用于长任务恢复 E2E：通过 LangGraph checkpointer 持久化 Deep Research 研究任务，演示 Web UI 里选择 LangGraph 状态快照后，通过 ResumeRun 沿用同一个 run_id 继续执行。它是一个多文件 LangGraph Agent 工程，不是单文件脚本；Web/API 路径使用自定义 Runner 接入真实 session event 和内置 Workspace 工具，离线路径保留 fixture，方便公开仓库用户 clone 后直接运行。
...
本地默认使用 `InMemorySaver` 做交互演示，不访问外部数据库；生产环境把 `LONG_TASK_RESUME_DEMO_MODE=postgres` 后会使用 `AsyncPostgresSaver` 和真实 PG。离线的 `demo.py` / `smoke.py` 仍使用 `workflow.py` 和 `tools.py` fixture，方便快速查看报告内容；真实 checkpoint resume / runtime cancel 请用 `agentengine run/web` 或部署到目标 pod 后验证。
...
| 检索补洞循环 | `deepresearch_gap_checker` + `deepresearch_query_expander` | 第一轮候选来源覆盖不足时，按证据缺口追加 gap-fill query 并再次检索。 |
...
Web UI 展示两类事件：主图业务安全点写入 `run_checkpoint` 索引，子图和工具节点通过 `stage_events` 转成 `tool_call` / `tool_result`。因此用户能看到 planner、query agent、gap checker、fetch agent、evidence screener、multi reviewer、critic 和 reporter 的真实执行过程；恢复区仍只展示可安全恢复的主图 checkpoint，避免把每个内部节点都暴露成恢复点。

## 产品体验参考

这个 demo 的用户体验按成熟 Deep Research Agent 的模式组织，而不是把恢复实现细节直接展示给用户：

- Open Deep Research 类实现通常先做 breadth/depth 拆解：生成检索问题、处理结果、提炼 learnings，再决定是否继续深挖。
- GPT Researcher 类实现通常拆成 planner、crawler/executor、publisher：先规划问题，再并行收集和摘要来源，最后聚合成有引用的报告。
- LangChain Open Deep Research 类实现把模型、搜索工具、最终报告模型都做成可配置项，并强调来源追踪和评测。

因此聊天区只展示研究主题、计划、进度、来源和报告；`run_id`、`checkpoint_id`、`receipt_key` 这类字段只保留在 session event 和会话恢复区里，供 ResumeRun/ListSessionCheckpoints/CancelRun 使用。

## 开发者学习路径

这个样例的重点不是固定某一个研究题目，而是教你写一个可恢复的长任务 Agent。建议按下面顺序阅读和改造：
...
3. 再看 `_build_web_search_subgraph`、`_expand_queries_for_gap` 和 `_build_analysis_subgraph`：这里演示 `Send` 并行 fan-out、gap-fill query 扩展、子图汇总和多 reviewer 分析。业务复杂度可以继续加子图，但主图恢复点要保持清晰。
4. 再看 `_runtime_event_pair`、`_stage_runtime_events` 和 `_append_runtime_stage_events`：内部工具节点不单写最终摘要，而是写入 `stage_events`，后台执行和 ResumeRun 都会把这些事件转发给 Web UI。
5. 再看 `_stream_checkpoint_resume`：ResumeRun 使用 `graph.astream(None, config=checkpoint_config, stream_mode="updates")` 续跑。`input=None` 是关键，它表示从 LangGraph checkpoint 继续，而不是重新提交用户问题。
6. 再看 `_write_workspace_artifact` / `_write_workspace_report_files`：每个阶段都通过 `ksadk.toolsets.workspace` 写入 Web UI Workspace；最终阶段同时写 `deepresearch-report.md` 和 `deepresearch-report.html`。
7. 最后看 `_run_background_job`：后台任务每个阶段完成后写入 `run_checkpoint` session event。这个事件只是 Web UI 和审计索引，里面保存 `framework_ref.langgraph`；真实状态仍在 LangGraph checkpointer。
...
| 接真实 LLM 分析 | `_call_required_llm`、`_run_analysis_stage`、`_run_write_report_stage` | Web/API 路径需要真实模型；失败时保留最近 checkpoint，恢复后从当前节点继续。 |
...
| 文件 | 作用 |
| --- | --- |
| `agent.py` | Web / API E2E 入口，自定义 `LongTaskE2ERunner`，本地使用 LangGraph `InMemorySaver`，PG 模式使用 `AsyncPostgresSaver`，并通过内置 Workspace 工具写研究产物。 |
...
| `e2e_http.py` | 本地/生产 HTTP E2E：流式启动后台任务、CancelRun、GetCheckpointResumePreview、ResumeRun。 |
...
如果本地访问 PG 超时，把同一目录部署到目标环境 pod 后再跑；目标环境 pod 能访问内网 PG 时，Web UI 才能展示真实 checkpoint 列表。
...
2. 发送“调研 GLP-1 受体激动剂在肥胖和 2 型糖尿病治疗中的临床证据、药物经济学、支付准入和真实世界安全性”。
3. Agent 会用真实 LLM 流式返回自然的启动确认；后台 run_id、invocation_id、checkpoint_id 只写入 session event 和恢复区，不暴露在普通聊天文本里。此后你可以继续普通对话；普通对话会调用模型，不会伪造 checkpoint。用户不需要说“执行长任务”，正常研究问题会触发后台 Deep Research。
4. Web UI 会订阅后台 invocation 的会话事件。后台任务会持续展示 `tool_call` / `tool_result`：规划、并行检索、gap-fill query 扩展、网页抓取、证据筛选、多 reviewer 分析、质检和 Workspace 写入。每完成一个业务安全点，才把 LangGraph checkpoint 的 `thread_id/checkpoint_id` 索引写入 `run_checkpoint` 事件，“会话恢复区”会实时出现状态快照和业务摘要。
...
1. `web_search`：优先调用 `DEEPRESEARCH_WEB_SEARCH_URL` 指向的搜索 API；未配置时尝试公开搜索页 `bing,sogou`，结果会带 `source` 字段；全部失败时才写入 `source=fallback` 的降级结果。第一轮覆盖不足时，`deepresearch_gap_checker` 会触发 `deepresearch_query_expander` 补一轮 gap-fill query。
2. `web_fetch`：对搜索结果 URL 发起 HTTP GET，清洗 HTML 的 `script/style/noscript` 后把正文片段写入 LangGraph state。
3. `llm.plan/analyze/critic/report`：配置 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL_NAME` 后走 OpenAI-compatible `/chat/completions`；Web/API 真实路径不会用写死文案伪造模型输出。
4. `write_workspace_file` / `write_workspace_files`：每个业务安全点都会把中间产物写入 Web UI Workspace 的 `research/<topic-slug>/` 目录。最终阶段会写入 `deepresearch-report.md` 和 `deepresearch-report.html`；恢复后通过 tool receipt 和 LangGraph checkpoint 避免重复外部工具调用。
...
1. 调 `/v1/responses` 流式启动后台 Deep Research。
2. 等待至少两个 LangGraph checkpoint 写入 session event。
3. 调 `CancelRun`，等待后台 invocation 进入 `cancelled`。
4. 调 `GetCheckpointResumePreview` 检查 run/checkpoint/receipt。
5. 调 `ResumeRun` 流式恢复，断言输出包含跳过阶段、`deepresearch-report.md` 和 `deepresearch-report.html`。
6. 调 `ListSessionCheckpoints`，断言 checkpoint 列表没有空 stage/status。

本地 `agentengine web` / `agentengine run` 场景可以让脚本先调 `CreateSession`。目标环境 pod 内验收时，`/v1/responses` 通常会用运行时真实 `agent_id` 自动创建 session；此时 action API 也必须使用同一个 runtime agent id，否则 PG session backend 会拒绝后续 checkpoint 查询或取消请求。推荐在 pod 内这样跑：
...
- `调研 GLP-1 受体激动剂在肥胖和 2 型糖尿病治疗中的临床证据、药物经济学、支付准入和真实世界安全性`
- `后台任务进度到哪了？`
- `普通对话：帮我解释一下 web_search 和 web_fetch 在 Deep Research 里的区别`
- `用户取消了刚才的 Deep Research 任务，现在状态应该怎么判断？`
...
- `tool_name`：工具名，例如 `web.search`、`web.fetch`、`write_workspace_file`、`write_workspace_files`。
- `run_id`：所属长任务。
...
- 未配置真实 web_search / web_fetch 时，Web/API 路径会先尝试公开搜索和 HTTP 抓取，失败后才降级为 fallback 来源。
- Workspace 默认使用本地 `AGENTENGINE_UI_DIR/workspace` 或 `.agentengine/ui/workspace`，不需要额外配置；模型未配置时 Web/API 真实 Deep Research 路径会失败，不会用写死报告冒充 LLM。
- 未配置模型 key 时，`demo.py` 和 `smoke.py` 仍可运行；`agentengine run/web` 的真实 Deep Research 路径需要 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL_NAME`，不会用写死输出冒充 LLM。
