...
渠道经营健康 Agent 演示如何围绕渠道表现、库存协同、价格治理、经营复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实经销商、合同价格、结算明细、客户名单、渠道返点规则或未公开促销策略。这是一个多文件 ADK Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 ADK 如何实现渠道经营健康 Agent。
...
```bash
cd 02-use-cases/channel-operations/channel-health-adk
cp ../../../.env.example .env
