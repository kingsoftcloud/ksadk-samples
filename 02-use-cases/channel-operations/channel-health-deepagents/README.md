# 渠道经营健康 Agent - DeepAgents

渠道经营健康 Agent 演示如何围绕渠道表现、库存协同、价格治理、经营复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实经销商、合同价格、结算明细、客户名单、渠道返点规则或未公开促销策略。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现渠道经营健康 Agent。
...
- `渠道表现`：用公开虚构数据展示当前状态。
- `库存协同`：把销售、库存和价格数据拆成可核验条目。
- `价格治理`：生成下一步行动，明确哪些需要人工复核。
- `经营复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/channel-operations/channel-health-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `渠道表现`、`库存协同`、`价格治理` 和 `经营复盘`。未接入真实系统时，输出会明确标注为 fixture，不会伪造平台结果。
...
- 把 `OPERATIONS_DATA` 接入真实渠道销售系统、库存系统、价格治理平台、经销商协同、促销系统和经营看板数据，但不要在样例输出中展示敏感字段。
