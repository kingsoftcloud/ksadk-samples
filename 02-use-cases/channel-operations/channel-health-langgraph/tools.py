...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为渠道销售系统、库存系统、价格治理平台、经销商协同、促销系统和经营看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为渠道销售系统、库存系统、价格治理平台、经销商协同、促销系统和经营看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责渠道表现、库存协同、价格治理和经营复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [
        {"step": index, "owner": '渠道运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"}
