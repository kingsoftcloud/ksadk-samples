# 销售运营 Pipeline Agent - LangChain

演示销售运营 Agent 如何把线索、阶段、价值、跟进记录和成交风险组织成下周 pipeline 行动计划。默认使用公开 fixture，不访问真实 CRM。 这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现 Sales Operations Pipeline Copilot。
...
```bash
cd 02-use-cases/sales-operations/pipeline-copilot-langchain
cp ../../../.env.example .env
