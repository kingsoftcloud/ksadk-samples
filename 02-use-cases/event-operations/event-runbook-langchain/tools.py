...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为活动报名系统、签到系统、场地资源、物料清单、讲师日程、问卷反馈和转化看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为活动报名系统、签到系统、场地资源、物料清单、讲师日程、问卷反馈和转化看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责活动报名、现场执行、资源调度和效果复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [
        {"step": index, "owner": '活动运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"}
