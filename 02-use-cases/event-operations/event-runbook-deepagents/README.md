# 活动会务运营 Agent - DeepAgents

活动会务运营 Agent 演示如何围绕活动报名、现场执行、资源调度、效果复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实参会者、手机号、签到明细、身份证件、赞助合同、嘉宾联系方式或未公开活动预算。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现活动会务运营 Agent。
...
- `活动报名`：用公开虚构数据展示当前状态。
- `现场执行`：把报名、签到、场地和物料数据拆成可核验条目。
- `资源调度`：生成下一步行动，明确哪些需要人工复核。
...
```bash
cd 02-use-cases/event-operations/event-runbook-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `活动报名`、`现场执行`、`资源调度` 和 `效果复盘`。未接入真实系统时，输出会明确标注为 fixture，不会伪造平台结果。
...
- 把 `OPERATIONS_DATA` 接入真实活动报名系统、签到系统、场地资源、物料清单、讲师日程、问卷反馈和转化看板数据，但不要在样例输出中展示敏感字段。
