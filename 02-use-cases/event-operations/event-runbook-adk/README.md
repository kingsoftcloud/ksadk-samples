...
活动会务运营 Agent 演示如何围绕活动报名、现场执行、资源调度、效果复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实参会者、手机号、签到明细、身份证件、赞助合同、嘉宾联系方式或未公开活动预算。这是一个多文件 ADK Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 ADK 如何实现活动会务运营 Agent。
...
```bash
cd 02-use-cases/event-operations/event-runbook-adk
cp ../../../.env.example .env
