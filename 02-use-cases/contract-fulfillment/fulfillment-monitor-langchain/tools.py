...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为合同管理系统、项目管理系统、交付工单、验收材料库、客户确认记录和财务回款看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为合同管理系统、项目管理系统、交付工单、验收材料库、客户确认记录和财务回款看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责履约节点、风险预警、协同动作和验收复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [
        {"step": index, "owner": '合同履约负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"}
