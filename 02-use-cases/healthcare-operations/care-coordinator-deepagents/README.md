# 医疗运营护理协同 Agent - DeepAgents

医疗运营 Agent 如何围绕门诊流转、护理资源、协同计划和风险提醒生成公开可复现的运营建议。默认使用虚构脱敏 fixture，不包含真实患者、病历或医疗诊断。 这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 Healthcare Operations Care Coordinator Agent。
...
- `患者流转`：用公开虚构数据展示预约、候诊、检查和出院教育的流转状态。
- `资源瓶颈`：定位护理排班、诊室利用和检查预约里的拥堵点。
- `协同计划`：生成护士、医生、导诊和运营团队的协同动作。
- `风险提醒`：提示等待时间、随访遗漏和数据合规边界。
...
```bash
cd 02-use-cases/healthcare-operations/care-coordinator-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `患者流转`、`资源瓶颈`、`协同计划` 和 `风险提醒`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `CARE_FLOW` 替换为预约、排队、诊室和护理工作站的脱敏统计数据。
- 把 `RESOURCE_BOTTLENECKS` 接入运营看板、排班系统和检查预约系统。
- 把协同计划写回排班、任务看板或运营复盘文档；不要在样例中处理真实诊断。
