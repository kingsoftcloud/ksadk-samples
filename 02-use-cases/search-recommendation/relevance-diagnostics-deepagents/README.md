# 搜索推荐诊断 Agent - DeepAgents

搜索推荐相关性诊断 Agent 如何围绕查询意图、召回质量、排序诊断和反馈闭环生成公开可复现的相关性诊断方案。默认使用公开虚构 fixture，不包含真实用户、原始 query、点击日志、设备标识、排序权重、商业规则或反作弊逻辑。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现搜索推荐相关性诊断 Agent。
...
- `查询意图`：用公开虚构数据展示当前状态。
- `召回质量`：把资源、服务或环境数据拆成可核验条目。
- `排序诊断`：生成下一步行动，明确哪些需要人工复核。
- `反馈闭环`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/search-recommendation/relevance-diagnostics-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `查询意图`、`召回质量`、`排序诊断` 和 `反馈闭环`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为脱敏查询样本、召回摘要、排序特征解释、点击转化指标和人工标注结果中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实搜索日志平台、召回服务、排序服务、标注平台、AB 实验和指标看板数据，但不要在样例输出中展示敏感字段。
