...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为搜索日志平台、召回服务、排序服务、标注平台、AB 实验和指标看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为搜索日志平台、召回服务、排序服务、标注平台、AB 实验和指标看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责查询意图、召回质量、排序诊断和反馈闭环。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '搜索推荐负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
