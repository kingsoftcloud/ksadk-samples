...
搜索推荐相关性诊断 Agent 如何围绕查询意图、召回质量、排序诊断和反馈闭环生成公开可复现的相关性诊断方案。默认使用公开虚构 fixture，不包含真实用户、原始 query、点击日志、设备标识、排序权重、商业规则或反作弊逻辑。这是一个多文件 ADK Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 ADK 如何实现搜索推荐相关性诊断 Agent。
...
```bash
cd 02-use-cases/search-recommendation/relevance-diagnostics-adk
cp ../../../.env.example .env
