# 社群运营增长 Agent - DeepAgents

社群运营 Agent 如何围绕社群画像、内容日历、互动风险和转化复盘生成公开可复现的社群增长方案。 默认使用公开虚构社群运营 fixture，不包含真实群成员、聊天记录、手机号、私域标签或客户成交信息。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现社群运营增长 Agent。
...
- `社群画像`：用公开虚构数据展示当前状态。
- `内容日历`：把资源、服务或环境数据拆成可核验条目。
- `互动风险`：生成下一步行动，明确哪些需要人工复核。
...
```bash
cd 02-use-cases/community-operations/community-growth-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `社群画像`、`内容日历`、`互动风险` 和 `转化复盘`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为社群分层、内容计划、互动风险、活动表现和转化路径中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实社群聚合指标、内容互动和活动转化数据，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回内容日历、社群运营任务、知识库更新计划或活动复盘文档；关键结论必须保留人工确认。
