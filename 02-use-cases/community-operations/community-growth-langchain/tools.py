...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为社群平台、内容日历、知识库、活动报名、CRM 线索池和运营看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为社群平台、内容日历、知识库、活动报名、CRM 线索池和运营看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责社群画像、内容日历、互动风险和转化复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '社群运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
