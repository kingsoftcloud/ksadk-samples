...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为交易风控、规则引擎、设备指纹、人工复核台和审计系统。
    """
...
def _render_summary() -> str:
    return (
        f"- 风险事件：{SCENARIO_CONTEXT['target']}\n"
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为交易风控、规则引擎、设备指纹、人工复核台和审计系统。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责交易异常、风险规则、人工复核和审计留痕。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '金融风控分析师', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
