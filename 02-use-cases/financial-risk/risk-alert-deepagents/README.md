# 金融风控风险预警 Agent - DeepAgents

金融风控 Agent 如何围绕交易异常、风险规则、人工复核和审计留痕生成公开可复现的风险处置方案。 默认使用公开虚构金融风控 fixture，不包含真实账户、真实交易、真实客户、卡号、证件号或内部风控规则。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 金融风控风险预警 Agent。
...
- `交易异常`：用公开虚构数据展示当前状态。
- `风险规则`：把资源、服务或环境数据拆成可核验条目。
- `人工复核`：生成下一步行动，明确哪些需要人工复核。
- `审计留痕`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/financial-risk/risk-alert-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `交易异常`、`风险规则`、`人工复核` 和 `审计留痕`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为交易摘要、规则命中、设备信号、客户风险等级和复核记录中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实风控信号、规则命中摘要和人工复核状态，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回人工复核队列、风险工单、客户触达任务或审计记录；关键结论必须保留人工确认。
