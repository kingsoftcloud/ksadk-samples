# 金融风控风险预警 Agent - LangGraph

金融风控 Agent 如何围绕交易异常、风险规则、人工复核和审计留痕生成公开可复现的风险处置方案。 默认使用公开虚构金融风控 fixture，不包含真实账户、真实交易、真实客户、卡号、证件号或内部风控规则。这是一个多文件 LangGraph Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangGraph 如何实现 金融风控风险预警 Agent。
...
```bash
cd 02-use-cases/financial-risk/risk-alert-langgraph
cp ../../../.env.example .env
