...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为事件平台、热线系统、现场巡查、交通协同、医疗联动和公众通知平台。
    """
...
def _render_summary() -> str:
    return (
        f"- 安全事件：{SCENARIO_CONTEXT['target']}\n"
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为事件平台、热线系统、现场巡查、交通协同、医疗联动和公众通知平台。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责事件线索、资源联动、风险通报和处置复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '公共安全联动负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
