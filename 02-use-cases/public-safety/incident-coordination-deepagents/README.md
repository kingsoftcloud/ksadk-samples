# 公共安全事件联动 Agent - DeepAgents

公共安全 Agent 如何围绕事件线索、资源联动、风险通报和处置复盘生成公开可复现的协同处置方案。 默认使用公开虚构公共安全 fixture，不包含真实报警人、真实地址、真实执法记录、真实监控画面或敏感处置流程。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现公共安全事件联动 Agent。
...
- `事件线索`：用公开虚构数据展示当前状态。
- `资源联动`：把资源、服务或环境数据拆成可核验条目。
- `风险通报`：生成下一步行动，明确哪些需要人工复核。
- `处置复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/public-safety/incident-coordination-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `事件线索`、`资源联动`、`风险通报` 和 `处置复盘`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为事件线索、现场资源、风险等级、通报口径和处置记录中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实事件摘要、资源状态和公众反馈数据，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回事件工单、资源联动任务、公众通报或处置复盘文档；关键结论必须保留人工确认。
