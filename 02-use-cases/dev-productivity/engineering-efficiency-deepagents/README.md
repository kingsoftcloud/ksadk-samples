# 研发效能改进 Agent - DeepAgents

研发效能 Agent 如何围绕迭代数据生成瓶颈分析、改进计划和度量指标。默认使用公开 fixture，不访问真实代码库或 CI。 这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 Engineering Efficiency Copilot。
...
```bash
cd 02-use-cases/dev-productivity/engineering-efficiency-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `研发概览`、`瓶颈分析`、`改进计划` 和 `度量指标`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `TEAM_OVERVIEW` 替换为真实迭代、需求、缺陷和发布统计。
- 把 `BOTTLENECKS` 接入 Git、CI、Issue、代码评审和事故复盘数据。
- 把改进计划写回团队周报、研发效能看板或 OKR 跟踪系统。
