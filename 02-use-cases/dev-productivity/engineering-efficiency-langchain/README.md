# 研发效能改进 Agent - LangChain

研发效能 Agent 如何围绕迭代数据生成瓶颈分析、改进计划和度量指标。默认使用公开 fixture，不访问真实代码库或 CI。 这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现 Engineering Efficiency Copilot。
...
```bash
cd 02-use-cases/dev-productivity/engineering-efficiency-langchain
cp ../../../.env.example .env
