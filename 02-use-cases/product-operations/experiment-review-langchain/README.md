# 产品运营实验复盘 Agent - LangChain

产品运营 Agent 如何围绕实验数据生成分群洞察、优化动作和复盘指标。默认使用公开 fixture，不访问真实用户数据或埋点系统。 这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现 Product Operations Experiment Review。
...
```bash
cd 02-use-cases/product-operations/experiment-review-langchain
cp ../../../.env.example .env
