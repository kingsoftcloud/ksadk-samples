# 产品运营实验复盘 Agent - DeepAgents

产品运营 Agent 如何围绕实验数据生成分群洞察、优化动作和复盘指标。默认使用公开 fixture，不访问真实用户数据或埋点系统。 这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 Product Operations Experiment Review。
...
```bash
cd 02-use-cases/product-operations/experiment-review-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `实验概览`、`用户分群`、`优化动作` 和 `复盘指标`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `EXPERIMENT_SUMMARY` 替换为真实 A/B 实验、埋点和漏斗数据。
- 把 `USER_SEGMENTS` 接入用户分群、留存、转化和付费行为分析。
- 把优化动作写回增长实验平台、运营日历或消息触达系统。
