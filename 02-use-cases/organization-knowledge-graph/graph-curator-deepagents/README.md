# 组织知识图谱运营 Agent - DeepAgents

组织知识图谱运营 Agent 演示如何围绕实体抽取、关系校验、知识更新、图谱复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实员工关系、组织调整、客户项目、权限信息、未公开战略或敏感知识资产。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现组织知识图谱运营 Agent。
...
- `实体抽取`：用公开虚构数据展示当前状态。
- `关系校验`：把材料、指标和风险拆成可核验条目。
- `知识更新`：生成下一步行动，明确哪些需要人工复核。
- `图谱复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/organization-knowledge-graph/graph-curator-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `实体抽取`、`关系校验`、`知识更新` 和 `图谱复盘`。未接入真实系统时，输出会明确标注为 fixture，不会伪造平台结果。
...
- 把 `OPERATIONS_DATA` 接入真实文档库、知识图谱、组织目录、项目台账、专家库和图谱质量看板数据，但不要在样例输出中展示敏感字段。
