# 组织知识图谱运营 Agent - LangChain

组织知识图谱运营 Agent 演示如何围绕实体抽取、关系校验、知识更新、图谱复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实员工关系、组织调整、客户项目、权限信息、未公开战略或敏感知识资产。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现组织知识图谱运营 Agent。
...
```bash
cd 02-use-cases/organization-knowledge-graph/graph-curator-langchain
cp ../../../.env.example .env
