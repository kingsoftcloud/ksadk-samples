...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为文档库、知识图谱、组织目录、项目台账、专家库和图谱质量看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为文档库、知识图谱、组织目录、项目台账、专家库和图谱质量看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责实体抽取、关系校验、知识更新和图谱复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [
        {"step": index, "owner": '知识图谱运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"}
