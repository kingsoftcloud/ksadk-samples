# 采购协同供应商筛选 Agent - DeepAgents

演示采购协同 Agent 如何把采购需求、供应商评分、谈判事项和审批风险组织成采购方案。默认使用公开 fixture，不访问真实采购系统。 这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 Procurement Collaboration Vendor Selection。
...
```bash
cd 02-use-cases/procurement-collaboration/vendor-selection-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `采购需求`、`供应商对比`、`谈判计划` 和 `审批风险`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `PROCUREMENT_REQUESTS` 替换为采购申请、预算和验收标准。
- 把 `VENDORS` 接入供应商库、报价系统和安全审计资料。
- 把谈判计划和审批风险写回采购协同系统或审批流。
