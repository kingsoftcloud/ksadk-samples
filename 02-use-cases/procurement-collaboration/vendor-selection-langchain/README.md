# 采购协同供应商筛选 Agent - LangChain

演示采购协同 Agent 如何把采购需求、供应商评分、谈判事项和审批风险组织成采购方案。默认使用公开 fixture，不访问真实采购系统。 这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现 Procurement Collaboration Vendor Selection。
...
```bash
cd 02-use-cases/procurement-collaboration/vendor-selection-langchain
cp ../../../.env.example .env
