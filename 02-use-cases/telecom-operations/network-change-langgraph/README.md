# 通信运维网络割接 Agent - LangGraph

通信运维 Agent 如何围绕网络告警、容量分析、割接计划和客户影响生成公开可复现的运维协同方案。 默认使用公开虚构网络运维 fixture，不包含真实基站、真实客户、真实拓扑或运营商生产数据。这是一个多文件 LangGraph Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangGraph 如何实现 通信运维网络割接 Agent。
...
```bash
cd 02-use-cases/telecom-operations/network-change-langgraph
cp ../../../.env.example .env
