# 通信运维网络割接 Agent - DeepAgents

通信运维 Agent 如何围绕网络告警、容量分析、割接计划和客户影响生成公开可复现的运维协同方案。 默认使用公开虚构网络运维 fixture，不包含真实基站、真实客户、真实拓扑或运营商生产数据。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 通信运维网络割接 Agent。
...
- `网络告警`：用公开虚构数据展示当前状态。
- `容量分析`：把资源、服务或环境数据拆成可核验条目。
- `割接计划`：生成下一步行动，明确哪些需要人工复核。
- `客户影响`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/telecom-operations/network-change-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `网络告警`、`容量分析`、`割接计划` 和 `客户影响`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为网络告警、容量利用率、拓扑变更、客户影响面和割接审批记录中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实告警和容量指标，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回割接任务系统、值班工单或变更复盘文档；关键结论必须保留人工确认。
