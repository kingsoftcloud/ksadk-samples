...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为NMS、告警平台、CMDB、容量分析、客服工单和割接审批系统。
    """
...
def _render_summary() -> str:
    return (
        f"- 割接窗口：{SCENARIO_CONTEXT['target']}\n"
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为NMS、告警平台、CMDB、容量分析、客服工单和割接审批系统。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责网络告警、容量分析、割接计划和客户影响。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": "通信网络运维负责人", "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
