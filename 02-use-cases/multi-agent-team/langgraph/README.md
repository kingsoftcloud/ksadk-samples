# 多 Agent 团队（Multi-Agent Team）- LangGraph

模拟 Planner、Researcher、Builder、Reviewer 的协作，把复杂任务拆解并收口。这是一个多文件 LangGraph Agent 工程，不是单文件脚本；默认使用本地公开示例数据，所以 clone 后不配置模型也能看到完整演示。

## 适用场景

- 想学习如何把 Multi-Agent Team 写成可维护的 KSADK 工程。
...
```bash
cd 02-use-cases/multi-agent-team/langgraph
cp ../../../.env.example .env
