...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为点检系统、EAM、CMMS、传感器平台、备件库存和安全作业票系统。
    """
...
def _render_summary() -> str:
    return (
        f"- 巡检批次：{SCENARIO_CONTEXT['target']}\n"
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为点检系统、EAM、CMMS、传感器平台、备件库存和安全作业票系统。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责巡检任务、缺陷识别、维修联动和安全复核。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '工业巡检安全员', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
