# 工业巡检安全复核 Agent - LangChain

工业巡检 Agent 如何围绕巡检任务、缺陷识别、维修联动和安全复核生成公开可复现的巡检协同方案。 默认使用公开虚构工业巡检 fixture，不包含真实设备序列号、真实车间位置、真实工艺参数、生产配方或安全事故记录。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现工业巡检安全复核 Agent。
...
```bash
cd 02-use-cases/industrial-inspection/safety-patrol-langchain
cp ../../../.env.example .env
