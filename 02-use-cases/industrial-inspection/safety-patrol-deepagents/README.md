# 工业巡检安全复核 Agent - DeepAgents

工业巡检 Agent 如何围绕巡检任务、缺陷识别、维修联动和安全复核生成公开可复现的巡检协同方案。 默认使用公开虚构工业巡检 fixture，不包含真实设备序列号、真实车间位置、真实工艺参数、生产配方或安全事故记录。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现工业巡检安全复核 Agent。
...
- `巡检任务`：用公开虚构数据展示当前状态。
- `缺陷识别`：把资源、服务或环境数据拆成可核验条目。
- `维修联动`：生成下一步行动，明确哪些需要人工复核。
- `安全复核`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/industrial-inspection/safety-patrol-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `巡检任务`、`缺陷识别`、`维修联动` 和 `安全复核`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为巡检批次、异常读数、设备状态、维修资源和安全作业记录中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实巡检摘要、缺陷识别结果和维修联动状态，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回维修工单、二次点检任务、安全作业票或巡检复盘文档；关键结论必须保留人工确认。
