...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为售后工单系统、备件库存、工程师排班、物流查询、客服回访和知识库。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为售后工单系统、备件库存、工程师排班、物流查询、客服回访和知识库。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责售后工单、备件协同、服务时效和客户回访。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '售后服务负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
