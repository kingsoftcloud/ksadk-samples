...
直播运营场控 Agent 如何围绕直播场控、商品讲解、异常舆情和转化复盘生成公开可复现的直播运营方案。默认使用公开虚构 fixture，不包含真实观众、订单、手机号、直播间 ID、后台截图、聊天原文或未公开价格策略。这是一个多文件 ADK Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 ADK 如何实现直播运营场控 Agent。
...
```bash
cd 02-use-cases/live-commerce-operations/session-control-adk
cp ../../../.env.example .env
