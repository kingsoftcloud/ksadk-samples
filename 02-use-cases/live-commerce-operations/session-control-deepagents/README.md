# 直播运营场控 Agent - DeepAgents

直播运营场控 Agent 如何围绕直播场控、商品讲解、异常舆情和转化复盘生成公开可复现的直播运营方案。默认使用公开虚构 fixture，不包含真实观众、订单、手机号、直播间 ID、后台截图、聊天原文或未公开价格策略。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现直播运营场控 Agent。
...
- `直播场控`：用公开虚构数据展示当前状态。
- `商品讲解`：把资源、服务或环境数据拆成可核验条目。
- `异常舆情`：生成下一步行动，明确哪些需要人工复核。
...
```bash
cd 02-use-cases/live-commerce-operations/session-control-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `直播场控`、`商品讲解`、`异常舆情` 和 `转化复盘`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为直播节奏摘要、商品讲解记录、互动指标、舆情标签和转化漏斗中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实直播中控台、商品池、库存系统、客服工单、舆情看板和转化分析平台数据，但不要在样例输出中展示敏感字段。
