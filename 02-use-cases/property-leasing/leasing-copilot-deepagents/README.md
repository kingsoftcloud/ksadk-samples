# 物业招商转化 Agent - DeepAgents

物业招商转化 Agent 演示如何围绕招商线索、租赁方案、合同协同、转化复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实租户、联系人、手机号、报价、合同条款、房源底价、佣金规则或未公开招商策略。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现物业招商转化 Agent。
...
- `招商线索`：用公开虚构数据展示当前状态。
- `租赁方案`：把租户需求、面积预算和楼层资源拆成可核验条目。
- `合同协同`：生成下一步行动，明确哪些需要人工复核。
...
```bash
cd 02-use-cases/property-leasing/leasing-copilot-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `招商线索`、`租赁方案`、`合同协同` 和 `转化复盘`。未接入真实系统时，输出会明确标注为 fixture，不会伪造平台结果。
...
- 把 `OPERATIONS_DATA` 接入真实招商 CRM、楼宇资源台账、租赁方案库、合同管理系统、带看日程和转化分析看板数据，但不要在样例输出中展示敏感字段。
