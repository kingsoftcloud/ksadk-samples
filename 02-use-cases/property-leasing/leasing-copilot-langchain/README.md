# 物业招商转化 Agent - LangChain

物业招商转化 Agent 演示如何围绕招商线索、租赁方案、合同协同、转化复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实租户、联系人、手机号、报价、合同条款、房源底价、佣金规则或未公开招商策略。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现物业招商转化 Agent。
...
```bash
cd 02-use-cases/property-leasing/leasing-copilot-langchain
cp ../../../.env.example .env
