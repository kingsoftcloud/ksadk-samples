# 旅游服务行程恢复 Agent - LangChain

旅游服务 Agent 如何围绕行程变更、资源协调、客户通知和服务补偿生成公开可复现的服务恢复方案。 默认使用公开虚构旅游服务 fixture，不包含真实旅客、真实订单、真实证件或供应商结算数据。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现 旅游服务行程恢复 Agent。
...
```bash
cd 02-use-cases/travel-service/trip-recovery-langchain
cp ../../../.env.example .env
