# 旅游服务行程恢复 Agent - DeepAgents

旅游服务 Agent 如何围绕行程变更、资源协调、客户通知和服务补偿生成公开可复现的服务恢复方案。 默认使用公开虚构旅游服务 fixture，不包含真实旅客、真实订单、真实证件或供应商结算数据。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 旅游服务行程恢复 Agent。
...
- `行程变更`：用公开虚构数据展示当前状态。
- `资源协调`：把资源、服务或环境数据拆成可核验条目。
- `客户通知`：生成下一步行动，明确哪些需要人工复核。
- `服务补偿`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/travel-service/trip-recovery-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `行程变更`、`资源协调`、`客户通知` 和 `服务补偿`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为行程订单、航班/车次状态、供应商可用资源、客户标签和服务条款中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实行程变更和资源状态，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回客服工单、通知任务、供应商协调单或复盘文档；关键结论必须保留人工确认。
