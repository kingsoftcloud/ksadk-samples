...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为订单系统、航旅动态、酒店库存、车辆调度、客服工单和通知平台。
    """
...
def _render_summary() -> str:
    return (
        f"- 行程事件：{SCENARIO_CONTEXT['target']}\n"
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为订单系统、航旅动态、酒店库存、车辆调度、客服工单和通知平台。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责行程变更、资源协调、客户通知和服务补偿。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": "旅游服务运营经理", "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
