# 会议纪要行动跟踪 Agent - LangChain

会议纪要行动跟踪 Agent 演示如何围绕会议材料、行动项、决策追踪、纪要复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实参会人、手机号、会议录音原文、敏感决策、客户名称、未公开路线图或内部人事信息。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现会议纪要行动跟踪 Agent。
...
```bash
cd 02-use-cases/meeting-minutes/action-tracker-langchain
cp ../../../.env.example .env
