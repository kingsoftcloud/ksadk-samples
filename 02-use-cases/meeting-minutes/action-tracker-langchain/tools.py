...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为会议录音转写、议题材料、任务系统、决策台账、日历和纪要发布流。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为会议录音转写、议题材料、任务系统、决策台账、日历和纪要发布流。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [
        {"step": index, "owner": '会议运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"}
