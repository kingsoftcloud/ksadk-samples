# 会议纪要行动跟踪 Agent - DeepAgents

会议纪要行动跟踪 Agent 演示如何围绕会议材料、行动项、决策追踪、纪要复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实参会人、手机号、会议录音原文、敏感决策、客户名称、未公开路线图或内部人事信息。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现会议纪要行动跟踪 Agent。
...
- `会议材料`：用公开虚构数据展示当前状态。
- `行动项`：把材料、指标和风险拆成可核验条目。
- `决策追踪`：生成下一步行动，明确哪些需要人工复核。
- `纪要复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/meeting-minutes/action-tracker-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `会议材料`、`行动项`、`决策追踪` 和 `纪要复盘`。未接入真实系统时，输出会明确标注为 fixture，不会伪造平台结果。
...
- 把 `OPERATIONS_DATA` 接入真实会议录音转写、议题材料、任务系统、决策台账、日历和纪要发布流数据，但不要在样例输出中展示敏感字段。
