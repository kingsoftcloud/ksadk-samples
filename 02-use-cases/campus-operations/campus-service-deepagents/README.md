# 园区运营服务 Agent - DeepAgents

园区运营服务 Agent 演示如何围绕园区工单、能耗资源、访客安全、运营复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实租户、访客姓名、手机号、车牌、门禁记录、监控画面、能耗账单或安防事件细节。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现园区运营服务 Agent。
...
- `园区工单`：用公开虚构数据展示当前状态。
- `能耗资源`：把工单、能耗和访客安全事件拆成可核验条目。
- `访客安全`：生成下一步行动，明确哪些需要人工复核。
- `运营复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/campus-operations/campus-service-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `园区工单`、`能耗资源`、`访客安全` 和 `运营复盘`。未接入真实系统时，输出会明确标注为 fixture，不会伪造平台结果。
...
- 把 `OPERATIONS_DATA` 接入真实园区工单系统、能耗平台、访客系统、安防巡检、会议室预订、物业排班和运营看板数据，但不要在样例输出中展示敏感字段。
