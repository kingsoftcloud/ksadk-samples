...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为园区工单系统、能耗平台、访客系统、安防巡检、会议室预订、物业排班和运营看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为园区工单系统、能耗平台、访客系统、安防巡检、会议室预订、物业排班和运营看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责园区工单、能耗资源、访客安全和运营复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [
        {"step": index, "owner": '园区运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"}
