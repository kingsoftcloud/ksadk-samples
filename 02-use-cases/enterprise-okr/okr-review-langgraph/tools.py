...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为OKR 系统、项目管理系统、指标看板、周报、风险登记表和复盘文档。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为OKR 系统、项目管理系统、指标看板、周报、风险登记表和复盘文档。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责目标拆解、进度风险、协同动作和季度复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [
        {"step": index, "owner": '经营管理负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"}
