...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为商品主数据、库存价格系统、活动权益平台、客服知识库、导购会话和转化分析平台。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为商品主数据、库存价格系统、活动权益平台、客服知识库、导购会话和转化分析平台。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责需求澄清、商品对比、库存价格和导购转化。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '导购运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
