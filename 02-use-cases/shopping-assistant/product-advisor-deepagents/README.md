# 智能导购顾问 Agent - DeepAgents

智能导购顾问 Agent 如何围绕需求澄清、商品对比、库存价格和导购转化生成公开可复现的智能导购方案。默认使用公开虚构 fixture，不包含真实用户、手机号、订单、地址、支付记录、个性化画像或未公开促销策略。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现智能导购顾问 Agent。
...
- `需求澄清`：用公开虚构数据展示当前状态。
- `商品对比`：把资源、服务或环境数据拆成可核验条目。
- `库存价格`：生成下一步行动，明确哪些需要人工复核。
- `导购转化`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/shopping-assistant/product-advisor-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `需求澄清`、`商品对比`、`库存价格` 和 `导购转化`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为脱敏导购咨询、商品属性摘要、库存价格状态、活动权益和转化漏斗指标中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实商品主数据、库存价格系统、活动权益平台、客服知识库、导购会话和转化分析平台数据，但不要在样例输出中展示敏感字段。
