# 地产营销线索转化 Agent - LangChain

地产营销 Agent 如何围绕楼盘线索、渠道投放、内容审核和转化复盘生成公开可复现的线索转化方案。 默认使用公开虚构地产营销 fixture，不包含真实客户、真实手机号、真实楼盘合同、真实成交价格或渠道后台数据。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现地产营销线索转化 Agent。
...
```bash
cd 02-use-cases/real-estate-marketing/lead-conversion-langchain
cp ../../../.env.example .env
