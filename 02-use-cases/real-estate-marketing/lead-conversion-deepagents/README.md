# 地产营销线索转化 Agent - DeepAgents

地产营销 Agent 如何围绕楼盘线索、渠道投放、内容审核和转化复盘生成公开可复现的线索转化方案。 默认使用公开虚构地产营销 fixture，不包含真实客户、真实手机号、真实楼盘合同、真实成交价格或渠道后台数据。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现地产营销线索转化 Agent。
...
- `楼盘线索`：用公开虚构数据展示当前状态。
- `渠道投放`：把资源、服务或环境数据拆成可核验条目。
- `内容审核`：生成下一步行动，明确哪些需要人工复核。
- `转化复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/real-estate-marketing/lead-conversion-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `楼盘线索`、`渠道投放`、`内容审核` 和 `转化复盘`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为楼盘项目、线索分层、渠道预算、内容素材和到访记录中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实线索摘要、渠道投放数据和内容审核状态，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回CRM 跟进任务、投放调整单、内容审核工单或转化复盘文档；关键结论必须保留人工确认。
