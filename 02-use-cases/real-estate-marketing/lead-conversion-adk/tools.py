...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为CRM、广告投放平台、案场系统、内容审核流和成交复盘看板。
    """
...
def _render_summary() -> str:
    return (
        f"- 营销活动：{SCENARIO_CONTEXT['target']}\n"
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为CRM、广告投放平台、案场系统、内容审核流和成交复盘看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责楼盘线索、渠道投放、内容审核和转化复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '地产营销负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
