...
数据治理 Agent 如何围绕数据资产生成质量问题、治理计划和责任矩阵。默认使用公开 fixture，不访问真实数据库或客户数据。 这是一个多文件 ADK Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 ADK 如何实现 Data Governance Quality Audit。
...
```bash
cd 02-use-cases/data-governance/quality-audit-adk
cp ../../../.env.example .env
