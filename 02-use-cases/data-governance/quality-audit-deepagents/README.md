# 数据治理质量审计 Agent - DeepAgents

数据治理 Agent 如何围绕数据资产生成质量问题、治理计划和责任矩阵。默认使用公开 fixture，不访问真实数据库或客户数据。 这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 Data Governance Quality Audit。
...
- `资产清单`：先说明数据表、用途和使用方。
- `质量问题`：列出缺失、延迟、口径和血缘问题。
- `治理计划`：给出可执行的数据治理动作。
- `责任矩阵`：明确平台、业务和数据负责人分工。
...
```bash
cd 02-use-cases/data-governance/quality-audit-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `资产清单`、`质量问题`、`治理计划` 和 `责任矩阵`。未接入真实系统时，输出会明确标注为 fixture。
...
- `基于当前样例数据，给我一份可执行的整改计划。`

## 接入真实能力

- 把 `DATA_ASSETS` 替换为真实数据目录、表元数据、血缘和使用方。
- 把 `QUALITY_ISSUES` 接入质量规则、调度日志、数据校验和工单系统。
- 把治理计划写回数据治理平台、数据 owner 看板或整改审批流。
