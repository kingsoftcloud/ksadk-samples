# 支付风控处置 Agent - DeepAgents

支付风控处置 Agent 如何围绕支付异常、风控规则、人工复核和审计留痕生成公开可复现的风险处置方案。默认使用公开虚构 fixture，不包含真实账户、卡号、手机号、身份证、交易号、设备指纹、支付流水或真实风控阈值。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现支付风控处置 Agent。
...
```bash
cd 02-use-cases/payment-risk/payment-guard-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `支付异常`、`风控规则`、`人工复核` 和 `审计留痕`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为脱敏支付异常样本、规则命中摘要、复核队列、申诉记录和审计日志中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实支付网关、风控规则引擎、设备指纹、退款系统、人工复核台和审计日志平台数据，但不要在样例输出中展示敏感字段。
