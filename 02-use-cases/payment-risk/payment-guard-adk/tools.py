...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为支付网关、风控规则引擎、设备指纹、退款系统、人工复核台和审计日志平台。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为支付网关、风控规则引擎、设备指纹、退款系统、人工复核台和审计日志平台。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责支付异常、风控规则、人工复核和审计留痕。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '支付风控负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
