...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为交通事件平台、公交调度、路况监测、停车诱导和公众通知平台。
    """
...
def _render_summary() -> str:
    return (
        f"- 交通事件：{SCENARIO_CONTEXT['target']}\n"
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为交通事件平台、公交调度、路况监测、停车诱导和公众通知平台。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责道路拥堵、运力调度、事件响应和公众通知。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '城市交通调度员', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
