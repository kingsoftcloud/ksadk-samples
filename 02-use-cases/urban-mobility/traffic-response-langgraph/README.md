# 城市交通事件响应 Agent - LangGraph

城市交通 Agent 如何围绕道路拥堵、运力调度、事件响应和公众通知生成公开可复现的出行协同方案。 默认使用公开虚构城市交通 fixture，不包含真实车牌、真实乘客、真实摄像头画面或城市级生产调度数据。这是一个多文件 LangGraph Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangGraph 如何实现 城市交通事件响应 Agent。
...
```bash
cd 02-use-cases/urban-mobility/traffic-response-langgraph
cp ../../../.env.example .env
