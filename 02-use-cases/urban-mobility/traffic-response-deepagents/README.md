# 城市交通事件响应 Agent - DeepAgents

城市交通 Agent 如何围绕道路拥堵、运力调度、事件响应和公众通知生成公开可复现的出行协同方案。 默认使用公开虚构城市交通 fixture，不包含真实车牌、真实乘客、真实摄像头画面或城市级生产调度数据。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 城市交通事件响应 Agent。
...
- `道路拥堵`：用公开虚构数据展示当前状态。
- `运力调度`：把资源、服务或环境数据拆成可核验条目。
- `事件响应`：生成下一步行动，明确哪些需要人工复核。
- `公众通知`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/urban-mobility/traffic-response-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `道路拥堵`、`运力调度`、`事件响应` 和 `公众通知`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为道路事件、实时路况、公交运行、运力资源和公众反馈中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实路况事件、运力状态和通知发布结果，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回交通事件看板、调度任务、公众通知或复盘文档；关键结论必须保留人工确认。
