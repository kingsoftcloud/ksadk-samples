# 客服 Agent（Customer Support）- LangGraph

把客户问题转成工单分级、知识匹配、处理步骤和升级建议。这是一个多文件 LangGraph Agent 工程，不是单文件脚本；默认使用本地公开示例数据，所以 clone 后不配置模型也能看到完整演示。

## 适用场景

- 想学习如何把 Customer Support 写成可维护的 KSADK 工程。
...
```bash
cd 02-use-cases/customer-support/langgraph
cp ../../../.env.example .env
