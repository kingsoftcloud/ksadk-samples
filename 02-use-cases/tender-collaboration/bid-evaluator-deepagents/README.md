# 招投标协同评审 Agent - DeepAgents

招投标协同评审 Agent 演示如何围绕招标需求、供应商响应、评分协同、风险复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实供应商、报价、投标文件原文、评分细节、评委身份、商业秘密或未公开采购计划。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现招投标协同评审 Agent。
...
- `招标需求`：用公开虚构数据展示当前状态。
- `供应商响应`：把需求条款、供应商响应和评审证据拆成可核验条目。
- `评分协同`：生成下一步行动，明确哪些需要人工复核。
- `风险复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/tender-collaboration/bid-evaluator-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `招标需求`、`供应商响应`、`评分协同` 和 `风险复盘`。未接入真实系统时，输出会明确标注为 fixture，不会伪造平台结果。
...
- 把 `OPERATIONS_DATA` 接入真实招标需求库、供应商响应文档、评分表、合规规则库、评审会议纪要和采购审批系统数据，但不要在样例输出中展示敏感字段。
