...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为招标需求库、供应商响应文档、评分表、合规规则库、评审会议纪要和采购审批系统。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为招标需求库、供应商响应文档、评分表、合规规则库、评审会议纪要和采购审批系统。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责招标需求、供应商响应、评分协同和风险复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [
        {"step": index, "owner": '采购评审负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"}
