...
招投标协同评审 Agent 演示如何围绕招标需求、供应商响应、评分协同、风险复盘生成公开可复现的业务复盘方案。默认使用公开虚构 fixture，不包含真实供应商、报价、投标文件原文、评分细节、评委身份、商业秘密或未公开采购计划。这是一个多文件 ADK Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 ADK 如何实现招投标协同评审 Agent。
...
```bash
cd 02-use-cases/tender-collaboration/bid-evaluator-adk
cp ../../../.env.example .env
