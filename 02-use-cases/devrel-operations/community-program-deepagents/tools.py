...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为GitHub Issues、Discussions、社群反馈、文档访问分析、活动报名和贡献者看板。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为GitHub Issues、Discussions、社群反馈、文档访问分析、活动报名和贡献者看板。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责开发者反馈、内容运营、活动节奏和社区复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": 'DevRel 负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
