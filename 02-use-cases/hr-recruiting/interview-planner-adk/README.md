...
演示招聘 Agent 如何把岗位要求、候选人摘要、面试轮次和录用风险组织成面试计划。默认使用公开 fixture，不访问真实招聘系统或个人简历。 这是一个多文件 ADK Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 ADK 如何实现 HR Recruiting Interview Planner。
...
```bash
cd 02-use-cases/hr-recruiting/interview-planner-adk
cp ../../../.env.example .env
