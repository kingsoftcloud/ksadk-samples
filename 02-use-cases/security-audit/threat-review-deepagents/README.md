# 安全审计威胁评审 Agent - DeepAgents

安全审计 Agent 如何围绕一次平台变更生成威胁分析、整改计划和验证证据。默认使用公开 fixture，不访问真实漏洞或生产环境。 这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 Security Audit Threat Review。
...
```bash
cd 02-use-cases/security-audit/threat-review-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `变更摘要`、`威胁分析`、`整改计划` 和 `验证证据`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `CHANGE_SUMMARY` 替换为真实变更单、架构图、接口和资产清单。
- 把 `THREATS` 接入 SAST、DAST、依赖扫描、权限审计和安全基线。
- 把整改计划和验证证据写回安全审计系统、发布审批流或风险台账。
