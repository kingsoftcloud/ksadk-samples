# 门店服务改进 Agent - DeepAgents

门店服务顾问 Agent 如何围绕门店咨询、服务承诺、问题升级和体验复盘生成公开可复现的服务改进方案。默认使用公开虚构 fixture，不包含真实顾客、手机号、会员号、订单、门店监控、聊天原文或赔付记录。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现门店服务顾问 Agent。
...
- `门店咨询`：用公开虚构数据展示当前状态。
- `服务承诺`：把资源、服务或环境数据拆成可核验条目。
- `问题升级`：生成下一步行动，明确哪些需要人工复核。
- `体验复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/store-service/store-advisor-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `门店咨询`、`服务承诺`、`问题升级` 和 `体验复盘`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为门店咨询摘要、服务承诺记录、升级工单、回访结果和知识库命中情况中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实门店 POS、会员服务台、客服工单、知识库、排班系统和体验回访看板数据，但不要在样例输出中展示敏感字段。
