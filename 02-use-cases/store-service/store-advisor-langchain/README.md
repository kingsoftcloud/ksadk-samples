# 门店服务改进 Agent - LangChain

门店服务顾问 Agent 如何围绕门店咨询、服务承诺、问题升级和体验复盘生成公开可复现的服务改进方案。默认使用公开虚构 fixture，不包含真实顾客、手机号、会员号、订单、门店监控、聊天原文或赔付记录。这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现门店服务顾问 Agent。
...
```bash
cd 02-use-cases/store-service/store-advisor-langchain
cp ../../../.env.example .env
