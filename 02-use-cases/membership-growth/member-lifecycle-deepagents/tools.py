...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为CRM、会员中心、权益平台、营销触达系统、订单分析和客服反馈平台。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为CRM、会员中心、权益平台、营销触达系统、订单分析和客服反馈平台。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责会员分层、权益推荐、触达节奏和留存复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '会员运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
