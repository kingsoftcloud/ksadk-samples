# 合规审阅 Agent - DeepAgents

演示合规审阅 Agent 如何把对外发布材料、风险规则、整改建议和审阅记录组织成可追溯流程。默认使用公开 fixture，不访问真实法务或审计系统。 这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 Compliance Review Copilot。
...
```bash
cd 02-use-cases/compliance-review/policy-review-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `材料清单`、`风险条款`、`整改建议` 和 `审阅记录`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `MATERIALS` 替换为 Workspace 文档、CMS 草稿或发布材料。
- 把 `POLICY_RULES` 接入法务规则库、品牌规范和安全基线。
- 把 `REVIEW_LOG` 写入审计系统、审批流或发布记录。
