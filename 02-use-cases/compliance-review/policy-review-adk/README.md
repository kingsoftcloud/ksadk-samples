...
演示合规审阅 Agent 如何把对外发布材料、风险规则、整改建议和审阅记录组织成可追溯流程。默认使用公开 fixture，不访问真实法务或审计系统。 这是一个多文件 ADK Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 ADK 如何实现 Compliance Review Copilot。
...
```bash
cd 02-use-cases/compliance-review/policy-review-adk
cp ../../../.env.example .env
