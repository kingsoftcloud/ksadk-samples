# 游戏运营版本复盘 Agent - DeepAgents

游戏运营 Agent 如何围绕玩家反馈、活动配置、经济系统监控和版本复盘生成公开可复现的运营复盘方案。 默认使用公开虚构游戏运营 fixture，不包含真实玩家 ID、充值记录、反作弊规则、内部活动配置或未公开版本计划。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现游戏运营版本复盘 Agent。
...
- `玩家反馈`：用公开虚构数据展示当前状态。
- `活动配置`：把资源、服务或环境数据拆成可核验条目。
- `经济系统监控`：生成下一步行动，明确哪些需要人工复核。
- `版本复盘`：说明合规、安全、经营或服务风险的控制方式。
...
```bash
cd 02-use-cases/game-operations/liveops-review-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `玩家反馈`、`活动配置`、`经济系统监控` 和 `版本复盘`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `SCENARIO_CONTEXT` 替换为玩家反馈、活动目标、奖励配置、经济指标和版本计划中的脱敏摘要。
- 把 `OPERATIONS_DATA` 接入真实反馈标签、活动转化和经济系统监控摘要，但不要在样例输出中展示敏感字段。
- 把 `ACTION_ITEMS` 写回运营复盘、配置调整审批、公告草稿或版本需求池；关键结论必须保留人工确认。
