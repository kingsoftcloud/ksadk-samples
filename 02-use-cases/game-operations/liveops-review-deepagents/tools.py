...
def load_scenario_context(query: str) -> dict:
    ...
    真实项目里这里可以替换为玩家反馈平台、活动配置、数据看板、经济系统监控、公告系统和版本管理。
    """
...
{goal_line} 当前 demo 使用本地公开 fixture，适合替换为玩家反馈平台、活动配置、数据看板、经济系统监控、公告系统和版本管理。
...
- `agent.py` 暴露框架入口。
- `tools.py` 负责玩家反馈、活动配置、经济系统监控和版本复盘。
...
def plan_actions(query: str, evidence: list[dict]) -> list[dict]:
    ...
    return [{"step": index, "owner": '游戏运营负责人', "action": item, "why": "把本地 fixture 转成可执行、可审查的下一步。"} for index, item in enumerate(ACTION_ITEMS, 1)]
