# 游戏运营版本复盘 Agent - LangGraph

游戏运营 Agent 如何围绕玩家反馈、活动配置、经济系统监控和版本复盘生成公开可复现的运营复盘方案。 默认使用公开虚构游戏运营 fixture，不包含真实玩家 ID、充值记录、反作弊规则、内部活动配置或未公开版本计划。这是一个多文件 LangGraph Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangGraph 如何实现游戏运营版本复盘 Agent。
...
```bash
cd 02-use-cases/game-operations/liveops-review-langgraph
cp ../../../.env.example .env
