...
def list_workspace_files(query: str) -> list[dict]:
    ...
    ADK 运行时会把这个函数作为工具暴露给 Agent；真实项目中应替换为 Workspace 工具调用。
    """
...
def render_demo_answer(query: str) -> str:
    ...
    result_lines = '\n'.join(
        f"- {item['purpose']}：当前 ADK demo 未连接真实 Sandbox，建议接入后执行 `{item['command']}` 并记录 exit code。"
...
{goal_line} ADK 版本把 Workspace 和 Sandbox 能力包装成函数工具，适合业务侧逐步替换为真实平台工具。
