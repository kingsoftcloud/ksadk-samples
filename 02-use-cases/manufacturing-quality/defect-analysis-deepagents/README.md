# 制造质量缺陷分析 Agent - DeepAgents

制造质量 Agent 如何围绕质量概览、缺陷归因、改进措施和验证指标生成公开可复现的质量改进计划。默认使用虚构产线 fixture，不包含真实工厂、真实客户、真实产品或商业机密。 这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 制造质量缺陷分析 Agent。
...
```bash
cd 02-use-cases/manufacturing-quality/defect-analysis-deepagents
cp ../../../.env.example .env
