# 教育培训学习辅导 Agent - DeepAgents

教育培训 Agent 如何根据学习目标、练习记录和测评结果生成个性化训练计划。默认使用公开虚构学习数据，不包含真实学生信息。 这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 DeepAgents 如何实现 Education Training Learning Coach Agent。
...
- `学习画像`：整理目标、当前水平、学习偏好和近期表现。
- `能力缺口`：从练习错误、知识点和时间投入里定位薄弱项。
- `训练计划`：给出分阶段练习、讲解、复盘和辅导动作。
- `评估方式`：设计可量化的测验、作业和复盘指标。
...
```bash
cd 02-use-cases/education-training/learning-coach-deepagents
cp ../../../.env.example .env
...
Web UI 中可以观察 Agent 如何输出 `学习画像`、`能力缺口`、`训练计划` 和 `评估方式`。未接入真实系统时，输出会明确标注为 fixture。
...
- 把 `LEARNER_PROFILE` 替换为 LMS、学习档案和培训目标。
- 把 `SKILL_GAPS` 接入练习记录、测评结果和讲师反馈。
- 把训练计划写回学习任务、日历或企业培训平台。
