# 教育培训学习辅导 Agent - LangChain

教育培训 Agent 如何根据学习目标、练习记录和测评结果生成个性化训练计划。默认使用公开虚构学习数据，不包含真实学生信息。 这是一个多文件 LangChain Agent 工程，不是单文件脚本；默认不依赖外部账号，适合作为开源样例和二次开发模板。

## 适用场景

- 想学习 LangChain 如何实现 Education Training Learning Coach Agent。
...
```bash
cd 02-use-cases/education-training/learning-coach-langchain
cp ../../../.env.example .env
