# AIOps 告警分诊 Agent - DeepAgents

演示 AIOps Agent 如何把指标告警、日志线索、变更事件和 runbook 组织成 incident triage。这是一个多文件 DeepAgents Agent 工程，不是单文件脚本；默认使用本地公开 fixture，所以 clone 后不配置监控系统也能看到完整分诊流程。
...
```bash
cd 02-use-cases/aiops/incident-triage-deepagents
cp ../../../.env.example .env
